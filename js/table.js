...
function buildStruct(strtable) {
  ...
  while ((current_char = strtable.charAt(i)) != "") {
    if (current_char == '\n') {
      ...
      if (strtable.substring(i - 6, i) == 'base64') {
...
function buildRowArray() {
  ...
  for (current_col = 0; dataArray[0][current_col] != null; current_col = current_col + 1) {
    var max_length = 0;
    for (current_row = 0; dataArray[current_row] != null; current_row = current_row + 1) {
      let cell = cellArray[current_row][current_col];
      if (cell.value.indexOf('base64') != -1) {
        var style = cell.value.substring(cell.value.lastIndexOf('(') + 1, cell.value.lastIndexOf(')')).split('-');
        var w = style[0];
        if (w / 10 > max_length) {
          max_length = w / 10;
        }
      } else if (strLen(cellArray[current_row][current_col].value) > max_length && cellArray[current_row][current_col].mergeAlign == false)
  ...
  for (current_row = 0; dataArray[current_row] != null; current_row = current_row + 1) {
    for (current_col = 0; dataArray[current_row][current_col] != null; current_col = current_col + 1) {
      ...
      if (cell.value.indexOf('base64') != -1) {
        ...
        var h = style[1];
        var image = new Image();
        cell.value = cell.value.substring(0, cell.value.lastIndexOf('('));
        image.src = cell.value;
        drawImage(image, cell.x + 1, cell.y + 1, w, h);
      } else {
